...
class Carrefour(Scraper):
    ...
    def prepare_cookie(self, storeDrive="886"):
    ...
    def get_data_from_url(self, url):
        ...
        nb_pages = self.get_nb_pages(html_content=response.text.encode('utf-8'))
    ...
    def next_page_script(self, url, nbpages, formDrive='0', timeout=120):
        ...
        for nb in range(nbpages):
            ...
            html_content = self.browser.html.encode('utf-8')
    ...
    def get_all_sections(self):
        ...
        soup = bs(response.text.encode('utf-8'), 'html5lib')
    ...
    def parse_page_product(self, filename):
        ...
        for name, unit, price in zip(corrected_names, units, prices):
            p = price.text.replace('Prix Promo : ', '').encode('utf-8').replace('â‚¬', '.') # Euro sign is not well decode, TODO: Need to be fixed
            n = name.span.text
            u = unit.text
